<script lang="ts">
	...
	const dimensions = [
		...
		'Raut Wajah', 'Kontak Mata', 'Ketahanan Stres (SRI)'
	];

	// ── DATA AGGREGATION ──
	const getDimensionScores = (sessionList: any[]) => {
		if (sessionList.length === 0) return Array(11).fill(0);
		
		const sum = (field: string) => sessionList.reduce((acc, s) => acc + (s[field] || 0), 0) / sessionList.length;
		// APE Pilar 1: sriScore idealnya datang langsung dari session_report;
		// fallback ke stressResistanceScore (skor LLM) bila belum tersedia di API.
		const sumSri = () =>
			sessionList.reduce((acc, s) => acc + (s.sriScore ?? s.stressResistanceScore ?? 50), 0) /
			sessionList.length;
		...
		return [
			...
			Math.round(sumSri()) // Ketahanan Stres (SRI)
		];
	};
	...
	const dimensionIcons: Record<string, any> = {
		...
		'Raut Wajah': Smile,
		'Kontak Mata': Eye,
		'Ketahanan Stres (SRI)': ShieldCheck
	};
