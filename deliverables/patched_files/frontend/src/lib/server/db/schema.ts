...
// APE Pilar 3: Kontrol Skenario Suasana (dipilih pengguna di halaman pra-sesi)
export const scenarioEnum = pgEnum('scenario_type', ['friendly', 'grilling', 'stress_test']);
...
export const userProfile = pgTable('user_profile', {
	...
	// ============================================================
	// APE (Adaptive Personalization Engine)
	// ============================================================
	// Pilar 1: Progresi Beban — dihitung di akhir tiap sesi, dipakai sebagai
	// default pressureLevel untuk sesi BERIKUTNYA.
	lastSri: real('last_sri'), // SRI numerik sesi terakhir (0-100)
	sriHistory: jsonb('sri_history'), // [{ sessionId, sri, level, at }]
	nextPressureLevel: integer('next_pressure_level').default(1).notNull(), // 0/1/2

	// Pilar 2: Profil Kelemahan Spesifik
	weaknessTags: jsonb('weakness_tags'), // string[], maks 3, terbaru menang
...
export const interviewSession = pgTable(
	'interview_session',
	{
		...
		// APE (Adaptive Personalization Engine)
		// ============================================================
		// Pilar 3: dipilih pengguna di halaman pra-sesi (disclaimer page).
		scenario: scenarioEnum('scenario').default('friendly').notNull(),
		// Pilar 1: snapshot level tekanan (0/1/2) diambil dari
		// userProfile.nextPressureLevel saat sesi dibuat. Sesi lama -> 1 (normal).
		pressureLevel: integer('pressure_level').default(1).notNull(),
...
export const sessionTurn = pgTable(
	'session_turn',
	{
		...
		questionAudioUrl: text('question_audio_url'), // URL file audio hasil TTS (edge_tts)
...
export const sessionReport = pgTable(
	'session_report',
	{
		...
		// APE Pilar 1: Stress Resilience Index terhitung (rule-based, lihat context.md),
		// BERBEDA dari stressResistanceScore (penilaian naratif LLM) di atas.
		sriScore: real('sri_score'),
...
export const interviewAvatar = pgTable('interview_avatar', {
	...
	// CATATAN AUDIT: engine TTS aktual saat ini adalah edge_tts (Microsoft Edge Neural TTS),
	// BUKAN Kokoro seperti disebut di proposal awal. ttsVoiceId berisi voice ID edge_tts,
	// contoh 'id-ID-ArdiNeural'. speed/pitch di params di bawah dipakai oleh APE Pilar 1 & 3.
	ttsVoiceId: text('tts_voice_id').notNull(), // ID voice edge_tts
