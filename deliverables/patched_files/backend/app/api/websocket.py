...
async def finish_and_report(websocket: WebSocket, db: Session, session: InterviewSession):
    ...
    if turns:
        ...
        # ── APE Pilar 1: Hitung Stress Resilience Index (SRI) ───────────────
        # tempo_bicara (wpm) rata-rata: perkirakan dari jumlah kata jawaban / durasi jawaban.
        # Jika data durasi per-jawaban tidak lengkap, fallback ke durasi sesi total.
        total_answer_words = sum(
            len((t.answerTranscript or "").split()) for t in turns
        )
        session_minutes = (session.durationSeconds or 0) / 60
        avg_wpm = (total_answer_words / session_minutes) if session_minutes > 0 else 150.0
        filler_per_minute = (total_fillers / session_minutes) if session_minutes > 0 else 0.0

        # Konsistensi 0-100 (dari LLM, dihitung di bawah setelah report_data) -> skala 0-5
        consistency_score_0_100 = report_data.get("dimensions", {}).get("consistency", {}).get(
            "score", report_data.get("overall_score", 50)
        )
        konsistensi_argumen_0_5 = (consistency_score_0_100 or 50) / 100 * 5

        sri_score = calculate_sri(
            filler_words_per_minute=filler_per_minute,
            tempo_bicara=avg_wpm,
            konsistensi_argumen=konsistensi_argumen_0_5,
        )
        next_pressure_level = compute_pressure_level(sri_score)

        # ── APE Pilar 2: Ekstrak weakness tags dari transkrip penuh sesi ────
        full_transcript_text = "\n".join(
            f"P: {t.questionText}\nJ: {t.answerTranscript or '(tidak menjawab)'}"
            for t in turns
        )
        new_weakness_tags = await extract_weakness_tags(full_transcript_text)
        ...
        if user_profile:
            ...
            # ── APE: simpan hasil Pilar 1 & Pilar 2 untuk dipakai sesi BERIKUTNYA ──
            user_profile.lastSri = sri_score
            user_profile.nextPressureLevel = next_pressure_level
            history = list(user_profile.sriHistory or [])
            history.append({
                "sessionId": session.id,
                "sri": sri_score,
                "level": next_pressure_level,
                "at": session.completedAt.isoformat() if session.completedAt else None,
            })
            user_profile.sriHistory = history[-50:]  # batasi riwayat agar kolom tidak membengkak

            if new_weakness_tags:
                # Tag terbaru menang; gabungkan dengan tag lama yang belum termuat, maks 3.
                merged = new_weakness_tags + [
                    t for t in (user_profile.weaknessTags or []) if t not in new_weakness_tags
                ]
                user_profile.weaknessTags = merged[:3]
...
async def send_next_question_stream(websocket: WebSocket, db: Session, session: InterviewSession, turn_num: int, user_transcript: str | None = None, current_question: str | None = None, phase_override=None):
    ...
    # APE Pilar 2: ambil weakness_tags dari riwayat profil pengguna (jika ada)
    weakness_tags = list(user_profile.weaknessTags) if user_profile and user_profile.weaknessTags else []
