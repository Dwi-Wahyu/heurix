				...
				<div class="mb-10 grid grid-cols-1 gap-6 lg:grid-cols-12">
					...
					>
						...
						<div class="mt-8 grid w-full grid-cols-2 gap-4">
							...
							{#if report.sriScore != null}
								<div
									class="col-span-2 rounded-2xl p-3 print:border print:border-gray-200 print:bg-white {report.sriScore >
									70
										? 'bg-green-50'
										: report.sriScore < 40
											? 'bg-red-50'
											: 'bg-amber-50'}"
								>
									<p class="mb-1 text-[10px] font-bold text-gray-400 uppercase">
										Stress Resilience Index (SRI)
									</p>
									<p
										class="text-lg font-bold {report.sriScore > 70
											? 'text-green-700'
											: report.sriScore < 40
												? 'text-red-700'
												: 'text-amber-700'}"
									>
										{Math.round(report.sriScore)} / 100
									</p>
									<p class="mt-1 text-[11px] text-gray-500">
										Sesi berikutnya akan {report.sriScore > 70
											? 'sedikit lebih menekan'
											: report.sriScore < 40
												? 'lebih suportif'
												: 'dengan tekanan yang sama'}, menyesuaikan performa Anda.
									</p>
								</div>
							{/if}
						</div>
