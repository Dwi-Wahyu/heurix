...
class UserProfile(Base):
    ...
    # Pilar 1: Progresi Beban — riwayat SRI & level tekanan yang akan dipakai
    # sebagai default untuk sesi BERIKUTNYA (dihitung di akhir sesi sebelumnya).
    lastSri = Column("last_sri", Float)  # SRI numerik sesi terakhir (0-100)
    sriHistory = Column("sri_history", JSON)  # list[{sessionId, sri, level, at}]
    nextPressureLevel = Column("next_pressure_level", Integer, default=1, nullable=False)  # 0/1/2

    # Pilar 2: Profil Kelemahan Spesifik
    weaknessTags = Column("weakness_tags", JSON)  # list[str], max ~3, terbaru menang
...
class InterviewSession(Base):
    ...
    scenario = Column(Enum(ScenarioType), default=ScenarioType.friendly, nullable=False)
    # Pilar 1: snapshot level tekanan yang dipakai di SESI INI, diambil dari
    # UserProfile.nextPressureLevel pada saat sesi dibuat. NULL/lama dianggap 1 (normal).
    pressureLevel = Column("pressure_level", Integer, default=1, nullable=False)
...
class SessionReport(Base):
    ...
    # APE Pilar 1: Stress Resilience Index terhitung (rumus rule-based di context.md),
    # BERBEDA dari stressResistanceScore (penilaian naratif LLM) di atas.
    sriScore = Column("sri_score", Float)
